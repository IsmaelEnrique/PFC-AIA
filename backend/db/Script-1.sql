...
-- 1. Agregar relación del carrito con el consumidor y comercio
ALTER TABLE carrito 
ADD COLUMN id_consumidor INT,
ADD COLUMN id_comercio INT,
ADD CONSTRAINT carrito_fk_consumidor FOREIGN KEY (id_consumidor) 
    REFERENCES consumidor(id_consumidor),
ADD CONSTRAINT carrito_fk_comercio FOREIGN KEY (id_comercio) 
    REFERENCES comercio(id_comercio);

-- 2. Agregar relación de items del carrito con variantes de productos
ALTER TABLE m_n_prod_carrito 
ADD COLUMN id_variante INT,
ADD CONSTRAINT m_n_prod_carrito_fk_prodvar FOREIGN KEY (id_variante) 
    REFERENCES variante(id_variante);

-- 3. Opcional: Agregar índice para mejorar performance en búsquedas
